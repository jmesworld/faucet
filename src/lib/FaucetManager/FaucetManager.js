...
class FaucetManager {
  ...
  async getBalance(address) {
    try {
      const [balance] = await this.lcdc.bank.balance(
  ...
  async sendTransaction(transactionParams = {}) {
    ...
    if (balance < amount) {
      throw new Error(`Insufficient amount ${balance}<${amount}`);
    }

    console.log(
      `Sending ${amount} to ${address} (self balance: ${balance} - target balance: ${recipientBalance})`
    );
